...
class MHDHelper():
    ...
    def _str2array(string):
        for t in [int, float, MHDHelper._str2bool]:
            try:
    ...
    def read_header(filename):
        ...
        header = {key: MHDHelper._str2array(value) for (key, value) in header.items()}
        return header
    ...
    def read_memmap(filename):
        ...
        dtype = np.dtype(MHDHelper._metatype2dtype_table[header['ElementType']])
    ...
    def read(filename):
        ...
        :param str filename: Image filename with extension mhd or mha.
        :return: ND image and meta data. (C, H, W)
        ...
        if data_filename == 'LOCAL':  # mha
            if data_is_compressed:
                ...
                numel = np.prod(np.array(MHDHelper._get_dim(header)))
                data_size = numel * np.dtype(MHDHelper._metatype2dtype_table[header['ElementType']]).itemsize
        ...
        if data_is_compressed:
            try:
                import pylibdeflate
                numel = np.prod(np.array(MHDHelper._get_dim(header)))
                decompressed_size = numel * np.dtype(MHDHelper._metatype2dtype_table[header['ElementType']]).itemsize
                data = pylibdeflate.zlib_decompress(data, decompressed_size)
            except:
                data = zlib.decompressobj().decompress(data)
        data = np.frombuffer(data, dtype=np.dtype(MHDHelper._metatype2dtype_table[header['ElementType']]))
        dim = MHDHelper._get_dim(header)
    ...
    def _check_header_sanity(header):
        spacing = MHDHelper._str2array(header['ElementSpacing'])
    ...
    def write(filename, image, header={}):
        ...
        # Construct header
        h = copy.deepcopy(MHDHelper._default_header)
        ...
        h['ElementType'] = MHDHelper._dtype2metatype_table[image.dtype.name]
        ...
        h = {key: MHDHelper._array2str(value) for (key, value) in h.items()}  # convert array into string if possible
